...
public class AradinRedisCacheWriter implements RedisCacheWriter {
	...
	AradinRedisCacheWriter(RedisConnectionFactory connectionFactory, Duration sleepTime, BatchStrategy batchStrategy, RedisCacheManagerProperties redisCacheManagerProperties) {
		this(connectionFactory, sleepTime, TtlFunction.persistent(), CacheStatisticsCollector.none(), batchStrategy, redisCacheManagerProperties);
	}
	 ...
	 * @param batchStrategy must not be {@literal null}.
	 */
	AradinRedisCacheWriter(RedisConnectionFactory connectionFactory, Duration sleepTime, TtlFunction lockTtl,
	...
	private Duration computeDuration(String name, Duration ttl) {
		long expiration = ttl.toMillis();
		Duration ttlOffset = getTtlOffset(name);
		if (shouldExpireWithin(ttlOffset)) {
			expiration += ThreadLocalRandom.current().nextLong(ttlOffset.toMillis());
	...
	public CompletableFuture<Void> store(String name, byte[] key, byte[] value, @Nullable Duration ttl) {
		...
		return asyncCacheWriter.store(name, key, value, computeDuration(name, ttl)) //
			...
			try {
				...
				if (shouldExpireWithin(ttl)) {
					put = ObjectUtils.nullSafeEquals(
							connection.stringCommands().set(key, value, computeExpiration(name, ttl), SetOption.ifAbsent()), true);
				} else {
	...
	public RedisCacheWriter withStatisticsCollector(CacheStatisticsCollector cacheStatisticsCollector) {
		return new AradinRedisCacheWriter(connectionFactory, sleepTime, lockTtl, cacheStatisticsCollector,
				this.batchStrategy, redisCacheManagerProperties);
	}
