package org.aradin.spring.velocity.tools;
...
import org.aradin.spring.velocity.ui.SpringResourceLoader;
...
public class ServletToolboxManager extends XMLToolboxManager
{
    ...
    {
        // little fix up
        if (!toolboxFile.startsWith("/") && !toolboxFile.startsWith("classpath:"))
        {
        ...
        if (toolboxManager == null)
        {
            ...
            try
            {
                ...
                if (is == null) {
                	log.debug("输入流为空，重新加载");
                	ResourceLoader resourceLoader = (ResourceLoader)(AradinBeanFactory.getBean(VelocityEngine.class).getApplicationAttribute(SpringResourceLoader.SPRING_RESOURCE_LOADER));
                	Resource resource = resourceLoader.getResource(toolboxFile);
                	if (resource != null) {
